...
Problems:
\begin{itemize}
    \item Broken instructions
    \item Unnecessarily complex code
    \item Uses outdated features (tristate buffers, reset)
    \item API can only print, not supply data structures
    \item No PCIe com module
\end{itemize}

Wanted features:
\begin{itemize}
    \item A functional system
    \item More modular and maintainable
    \item More configurable (not only powers of two) (for better resource utilization)
    \item Adaptive API
...
\TODO
Decided to rebuild the entire platform.
