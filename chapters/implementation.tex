...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.78\textwidth]{statemachine-dft}
...
Reset: Everything except for instruction memory is cleared.

All instructions (one function for each). LUTs and development rules are sent as structs.

Getter functions that parse data from the FPGA into data structures.

\subsection{Supplementary API}
...
Everything in libcarp is compiled to statically linked library (libcarp.a).

Save programs in programs/.
Include carp.h, optionally print.h and postscript.h.
Run make (automatic link).

Make test will run all programs whose name start with ``test\_''.
All tests use a common test framework for asserts (testframework.cinclude).
