...
The buffers are cleared once again on disconnect, in addition to the synthesis information.
The reset function can be used to set the system into freshly booted state by clearing all counters, sblocks, buffers and BRAMs except for the instruction memory \todo{clear instruction memory}.
...
For each instruction that should return data there is a corresponding getter function used to receive that data and parse it into data structures (where applicable).
The getter functions are needed because of the hardware platform's separate instruction memory and control flow, causing the amount of instructions issued by the API to not necessarily correspond to the number of times the instruction is executed.
