...
Index zero is reserved for representing that no rules have triggered.
...
When activated, it sends all parameters that might be useful to the API.
...
\begin{itemize}
    \item Sends words from fitness buffer
    \item Number of words is decided by the fitness module
\end{itemize}

\subsection{Rule Vector Reader}

\begin{itemize}
    \item Reads one or more rule vectors from development
    \item 32 hits per word (except maybe last)
    \item First (rule 0) always hits
\end{itemize}

\subsection{Rule Numbers Reader}

\begin{itemize}
    \item Reads rule activations for all cells during the last devstep
    \item Raster order, fits as many numbers into each word as possidle
    \item Word-aligns between rows and numbers that would be split
\end{itemize}

\subsection{Cell Writer Reader}

The Cell Writer Reader is easily the most complex control module.
This is due to the intricate interactions \todo{better word?} with the Cell Buffer required to change only selected cells.
Additionally, it must convert variable-width types and states into fixed-width words in an efficient manner.

\begin{itemize}
    \item Read is basicly like Rule Numbers Reader
